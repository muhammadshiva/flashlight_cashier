...
├── domain/
│   ├── entities/           # Plain Dart classes (business objects)
│   ├── usecases/          # Business logic implementations
│   └── repositories/      # Abstract repository interfaces
├── data/
│   ├── models/           # Freezed models with JSON serialization
│   ├── datasources/      # API/local data source implementations
...
class GetAllUseCase implements UseCase<List<Item>, NoParams> { ... }
```
...
- **API Response Envelope**: All API responses follow a standard format:
  ```json
  {
    "success": boolean,
    "message": string,
  ...
  ```
  Models parse this envelope and extract the `data` field (see `API_CONTRACT.md` for full specifications)
...
All data models use Freezed with this structure:
```dart
import 'package:freezed_annotation/freezed_annotation.dart';

part 'model_name.freezed.dart';  // Generated by freezed
part 'model_name.g.dart';        // Generated by json_serializable
...
```
**Important**: Data layer uses Models (with Freezed), Domain layer uses Entities (plain classes). Models have `toEntity()` methods for conversion.
...
- **Currency Formatting**: Use the `num` extension for consistent currency display: `price.toCurrencyFormat()` (formats as `Rp X,XXX` with Indonesian locale)

## Additional Documentation

- **API_CONTRACT.md**: Complete API endpoint specifications and request/response formats
- **POS_REQUIREMENTS.md**: Functional and non-functional requirements for the POS application